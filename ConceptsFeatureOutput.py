...
from ibm_watson.natural_language_understanding_v1 import Features,EmotionOptions,CategoriesOptions,ConceptsOptions,EntitiesOptions
import scrape_parse
import csv
import json
import os
...
with open('./Sample_Class_CSV_Files/' + file, newline='') as csvfile:
    ...
    OutputFile = open(fileName[0] + "_Concepts.txt","a+")
    for i in valuable:
        print(i)
        print("\n")
        OutputFile.write(i)
        OutputFile.write("\n")
        response = NLU.analyze(text = i, features = Features(concepts = ConceptsOptions(limit = 2))).get_result()
        print(json.dumps(response["concepts"],indent = 2))
        OutputFile.write(json.dumps(response["concepts"],indent = 2))
        OutputFile.write("\n")
    ...
    for j in needs_improvement:
        ...
        OutputFile.write(j)
        OutputFile.write("\n")
        response = NLU.analyze(text = j, features = Features(concepts = ConceptsOptions(limit = 2))).get_result()
        print(json.dumps(response["concepts"],indent = 2))
        OutputFile.write(json.dumps(response["concepts"],indent = 2))
        OutputFile.write("\n")   